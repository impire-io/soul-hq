...
- **Announcement:** `topic.announce` carries `"sealed": true`, cleartext `topic_id`, the epoch-1 material, and *encrypted* `name` and `subject_matter`. The realm sees that a sealed topic exists and who is expected — not what it is about. **The topic id derives from the word `sealed`, never from the display name** — a name-derived slug would leak the very name the announcement encrypts (build finding, [episode 0131](../../../04-JOURNEY/0131-core-sealed-topics-build.md)).
...
- **Mentions:** the library still fires `mention.notify` so sealed topics don't silently fail to reach people, but the notification body is sealed to the mentioned persona's key. The notification's *existence* leaks (threat model). A mentioned persona with no published sealing key is skipped with a warning — never notified in cleartext (build refusal, [episode 0131](../../../04-JOURNEY/0131-core-sealed-topics-build.md)).
