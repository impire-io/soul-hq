...
- **The vault bindings gate the data-dependent decisions.** A role is an
  account signing key bound to its account (D24; noun per D28 — a team is
  the account, the tenant); a persona key is bound to its owner (account,
  user) (D6 as amended); `sign.record` and `keys.public` check the caller
  against the binding; the binding-resolved mints resolve their signing
  key by the target account's binding (D5 as amended), the by-name mint
  by declared role name (D28).

What remains declared (D2's principle, relocated): token records name the
token lane's identities, bindings name roles and persona owners, permission
