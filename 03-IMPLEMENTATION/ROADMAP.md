...
- **M3.1 — first fleet milestone.** ✅ **Done 2026-08-19** ([episode
  ...
  **Next, now unblocked:** the declaration trigger vocabulary (research
  ...
  Build stays by demand; the enforcement-read gap `[O]` (widened agent
  scope vs runtime-side reads) is the first build decision, and
  mention-wake agents ship before topic-wake colonies (loop safety —
  the colony gate — graduated to design
  [`0006-loop-safety.md`](../02-DESIGN/soulstream-workloads/0006-loop-safety.md)
  and BUILT the same day in wrap, [episodes
  0128](../04-JOURNEY/0128-ecosystem-loop-safety.md)/[0129](../04-JOURNEY/0129-workloads-the-wake-budget-builds.md):
  a colony deployment carries the shipped budget at its dispatcher's
  admission seam).
