...
**The calm pass is SHIPPED — v0.11.0-rc.7, composed as the house's
v0.14.0-rc.8** ([episode
0157](../04-JOURNEY/0157-shell-the-calm-pass.md)): the operator
called the shipped canon looks-without-feel and directed a
screen-by-screen walk. One act leads each screen from the page head;
verified is silent in the thread while the record's own screen names
every verdict; Home is a glance of four uniform tiles, the
instruments on System status alone; messages are words on the paper
with the mention's ink shoulder; the two big creations are the kit's
stepped wizards (the declare limits unfolded — design 0009's own test
refused the fold); and a declared agent's home is started at declare
time when the board has none to offer. Zero act endpoints changed.
Amendments ledgered in 0007 §7, 0008 §6, 0009 §7; parked for the
operator: the storage record-detail slide-over, the Home title, the
Add/Declare key hierarchy.

**The second canon is HELD and SHIPPED — v0.11.0-rc.6** ([episodes
...
render gate fetching Geist. The operator called the canon held on the
published before/after screens; merged, tagged (shell `v0.11.0-rc.6`,
idp `v0.8.2`), and composed into the house as `v0.14.0-rc.7`.
...
8. **M-second-canon — the re-skin** (decided AND built 2026-08-29,
   ...
   0011 §11). ✅ All four gates of 0011 §9 paid: gates 1–3 measured
   green, gate 4 the operator's own call on the published
   before/after screens (episode
   [0156](../04-JOURNEY/0156-soulstream-the-rc7-carries-the-second-canon.md));
   shipped as `v0.11.0-rc.6`, the fold as idp `v0.8.2`, the house
   composing both as `v0.14.0-rc.7`.
