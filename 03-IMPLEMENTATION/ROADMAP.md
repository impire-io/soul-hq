...
`accounts.create` amending AUTH `allowed_accounts`. D47 leads the
build order because today's `accounts.*` family births an
admitted-but-inert tenant [measured] — a defect independent of the
topology. The product's halves (SystemConn wiring, an `accounts.*`
client surface) ride whichever product cycle adopts multi-tenancy.
