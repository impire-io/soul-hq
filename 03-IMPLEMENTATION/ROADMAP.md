...
| [soulstream-inference](#soulstream-inference--the-mind) | **Founded 2026-08-28** ([episode 0142](../04-JOURNEY/0142-ecosystem-the-inference-plane.md), design [`0001`](../02-DESIGN/soulstream-inference/0001-the-inference-plane.md)): the inference plane graduated with all five bars measured the day its research opened — capabilities as tools (zero scope widening), one client loop, the real harness through the door in 2.21s, names-not-routes, the record the only record | **M1 — the plane's first slice**: frame-grammar codec, instance kit with the adapter seam (stand-in + anthropic), client (collect + resolver + catalogue descriptor), the door; the graduation bars as standing tests; then the product wiring (catalogue home, door-key mint) and design 0007 §3's inference block resolving against the catalogue |
