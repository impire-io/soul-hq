...
`jwt_settings` coupling — the byon_live rig upgraded, the live run
the operator's one command), and the product's spec 012 wired
`SystemConn`, the persistent dir resolver, and the `soulstream
account` hand — create → usable admission 8.8ms through the running
house, surviving restart [measured].*
